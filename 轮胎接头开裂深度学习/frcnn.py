...
class FRCNN(object):
    _defaults = {
        "model_path": 'model_data/epoch009-loss0.013-rpn0.012-roi0.001.h5',
        # "model_path": 'logs/epoch004-loss0.228-rpn0.133-roi0.095.h5',
    ...
    def generate(self):
        ...
        # 画框设置不同的颜色
        hsv_tuples = [(x+1.5 / len(self.class_names), 1., 1.)
    ...
    def detect_image(self, image):
        ...
        # rpn_results = self.bbox_util.detection_out(preds, anchors, 1, confidence_threshold=0.9)
        rpn_results = np.array(self.bbox_util.detection_out(preds, anchors, 1, confidence_threshold=0))
        # 筛选出其中得分高于confidence的建议框 一般获得300个

        if len(rpn_results[0]) == 0:
            return image
