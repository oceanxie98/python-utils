...
class BBoxUtility(object):
    ...
    def encode_box(self, box, return_iou=True):
        ...
        # 找到对应的先验框，先验框与真实框进行编码，获得预测结果。根据预测结果对先验框调整获得真实框
    ...
    def ignore_box(self, box):
        ...
        # 找到每一个真实框，重合程度较高的先验框>0.3 <0.7的忽略
    ...
    def assign_boxes(self, boxes, anchors):
        ...
        # 对每一个真实框都进行iou计算，取得所有真实框需要忽略的先验框
    ...
    def detection_out(self, predictions, mbox_priorbox, num_classes, keep_top_k=300,
        ...
        # prdictions 就是 preds
        # confidence_threshold=0.5调最小的置信度   keep_top_k=300最后选出的数量
        ...
        for i in range(len(mbox_loc)):
            ...
            for c in range(num_classes):
                c_confs = mbox_conf[i, :, c]    # 框
                c_confs_m = c_confs > confidence_threshold  # 框的置信度
                if len(c_confs[c_confs_m]) > 0:
                    # 取出得分高于confidence_threshold的框
                    boxes_to_process = decode_bbox[c_confs_m]
                    confs_to_process = c_confs[c_confs_m]
                    # 进行iou的非极大抑制(找到重合区域较高的框，选择得分最高的框）
